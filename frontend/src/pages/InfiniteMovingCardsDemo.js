...
import { InfiniteMovingCards } from "../components/homepage/trans-cards.tsx";
...
export function InfiniteMovingCardsDemo() {
  ...
  useEffect(() => {
    axios
      .get("https://akap-api.vercel.app/api/get-all-transactions")
      .then((response) => {
        if (response.data.success) {
          setTransactions(response.data.data);
          console.log(response.data.data, "transac");
          const alltransactions = response.data.data;
          setTransactions(
            alltransactions?.sort((a, b) => {
              return (
                new Date(a.date_created["$date"]) -
                new Date(b.date_created["$date"])
              );
            })
          );
          // console.log(nice, "yeyy new sorted")
        }
      });
  }, []);
  ...
  return (
    <div className="h-[40rem] rounded-md flex flex-col antialiased bg-transparent items-center justify-center relative overflow-hidden">
      <h1 className="mb-9 mt-9 text-5xl text-center text-blue-600 font-bold">
        Recent Transactions
      </h1>
      <InfiniteMovingCards
        items={transactions}
