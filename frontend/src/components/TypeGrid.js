...
import { BentoGrid, BentoGridItem } from "./homepage/bento-grid.tsx";
import axios from "axios";

export function TypeGrid({ itemType }) {
  const [displayItems, setDisplayItems] = useState([]); // State to track items to display

  // Filtering function
  const filterItemsByType = (items, type) => {
    console.log(items, "hii im here type")
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        console.log(response.data.data, "typegrid, causes");
  ...
  }, [itemType]); // Run effect whenever itemType changes
...
// const medicalBillsItems = displayItems.filter((item) => item.type === itemType);
